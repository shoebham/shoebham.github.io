...
const preprocess = (text) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const previousNode = result.length
...
const transform = (nodes, level = 0, rawIndentation = 0) => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    const nextNode =
      i < nodes.length - 1 ? nodes[i + 1] : { value: "", indentation: -1 };
  ...
  for (const node of source) {
    ...
    if (isString(node)) {
      if (page) {
        ...
        if (
          pageElements.length &&
          pageElements[pageElements.length - 1].type === "text"
        ) {
...
const render = (data, document, root) => {
  ...
  const applyOption = (name, callback) => {
    if (name in data.options) {
      ...
      const valueArray = value.includes(",")
  ...
  // base style tag, containing the target-based multi-page logic only
  const baseStyleTag = makeElement(document, "style", {
    textContent: baseStyle,
  });
